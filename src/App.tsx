...
const App: React.FC<props> = () => {
  ...
  const [confirmSeed, setConfirmSeed] = useState<string[]>([]);
  const [phrase, setPhrase] = useState<string>("");
  const [whichModal, setWhichModal] = useState<Number>(0);
  const [showSeed, setShowSeed] = useState<Boolean>(false);
  const [showConfirmSeed, setShowConfirmSeed] = useState<Boolean>(false);
  ...
  const RandomConfirmSeedFunc = (): void => {
    const positions = [];

    for (let i = 0; i < 3; i ++) 
      positions.push(Math.floor(confirmSeed.length * Math.random()))
    
    const modifierSeed = seed.map((item, index) => {
      if (positions.includes(index)) return "*";
      else return item;
    })

    setConfirmSeed(modifierSeed);

    console.log("LSRLSR" + modifierSeed);
          ...
          <div className='absolute z-1 w-[300px] h-[500px] bg-gray-900 border-2 rounded-[20px] flex flex-col justify-start items-center gap-5 p-[10px]'>
            ...
            </div>
            {
              ...
              <div className='flex flex-col justify-center items-center gap-1'>
                ...
                <div className='flex flex-row gap-5 mt-[20px]'>
                  ...
                  <div className='max-w-[250px] rounded-lg bg-white text-black text-center text-[12px] px-5 py-3 font-bold' onClick={() => setWhichModal(1)}>
                    Set Password
                  </div>
                </div>
              </div>
            }
            {
              ...
              <div className='flex flex-col justify-center items-center text-[13px]'>
                <div className='flex flex-col border-l-2 rounded-xl pl-2'>
                  ...
                  <div className='italic text-[13px]'>If you forget your password, you will need to delete the chat and start over again.</div>
                </div>
                <div className='flex flex-col w-[90%] rounded-lg'>
                  <div className='flex flex-col justify-center items-start'>
                      <InputPassword inputName={"Input your Password"} password={password} setPassword={setPassword} setMatchPassword={setMatchPassword}/>
                  </div>
                </div>

                <div className='flex flex-row gap-5 mt-[20px]'>
                  <div className='max-w-[250px] rounded-lg bg-white text-black text-center text-[12px] px-5 py-3 font-bold' onClick={() => setWhichModal(0)}>
                    Back
                  </div>
                  <div className={`max-w-[250px] rounded-lg bg-white text-black text-center text-[12px] px-5 py-3 font-bold  ${!matchPassword&&"opacity-50 pointer-events-none cursor-not-allowed"}`} onClick={() => {setWhichModal(2); RandomConfirmSeedFunc()}}>
                    Confirm Seed Phrase
                  </div>
                </div>
              </div>
            }
            {
              ...
              <div className='flex flex-col justify-center items-center gap-1'>
                ...
                <div className='mt-[5px] flex flex-row flex-wrap justify-around items-center py-[30px] gap-3 border-2 border-white rounded-lg relative'>
                  {
                    confirmSeed.map((item, index) => {
                      return (
                        <div className={`flex flex-row`}>
                          <div className='mr-1 w-[20px] text-right'>{`${index + 1}. `}</div>
                          <div className={`w-[100px] bg-white flex justify-center items-center text-[15px] text-black rounded-lg ${item && !showConfirmSeed && "select-none blur-sm"}`}>{item=="*"?"":item}</div>
                        </div>
                      )
                    })
                  }
                  <div className='absolute flex flex-col gap-2 text-[15px] justify-center items-center'>
                    <img src={showConfirmSeed ? EyeOffIcon : EyeOnIcon} onClick={() => setShowConfirmSeed(!showConfirmSeed)} className='bg-gray-400 rounded-full text-white w-[35px] h-[30px] cursor-pointer' />
                  </div>
                </div>
                <div className='flex flex-row gap-5 mt-[20px]'>
                  <div className='max-w-[250px] rounded-lg bg-white text-black text-center text-[12px] px-5 py-3 font-bold' onClick={() => RandomConfirmSeedFunc()}>
                    Reset
                  </div>
                  <div className='max-w-[250px] rounded-lg bg-white text-black text-center text-[12px] px-5 py-3 font-bold' onClick={() => setWhichModal(3)}>
                    Create Wallet
                  </div>
                </div>
              </div>
